...
void filter_16bit_fir()
{
  ...
  /**
   * Declare the required filter struct, xs3_filter_fir_s16_t.
   * 
  ...
  /**
   * Initialize filter with xs3_filter_fir_s16_init().
   * 
   * The parameters here are straight-forward, given the description of filter behavior
   * above.
   */
  xs3_filter_fir_s16_init(&filter, sample_buffer, TAP_COUNT, coefficients, filter_shr);
  ...
  /**
   * Using xs3_filter_s16_add_sample(), supply the filter with random input samples without actually
  ...
  for(int k = -TAP_COUNT; k < 0; k++) {
    ...
    xs3_filter_fir_s16_add_sample(&filter, input_sample);
  ...
  /**
   * Using xs3_filter_s16(), supply the filter with new random input samples, computing the filtered
   * output value each time.
   */
  for(int k = 0; k < FILTER_TICKS; k++){ 
    int16_t input_sample = (int16_t) rand();
    int16_t output_sample = xs3_filter_fir_s16(&filter, input_sample);
