...
TEST_GROUP_RUNNER(vect_complex_mul) {
  ...
  RUN_TEST_CASE(vect_complex_mul, vect_complex_s32_mul_prepare);
  RUN_TEST_CASE(vect_complex_mul, vect_complex_s16_mul_basic);
  RUN_TEST_CASE(vect_complex_mul, vect_complex_s16_mul_random);
  RUN_TEST_CASE(vect_complex_mul, vect_complex_s32_mul_basic);
  RUN_TEST_CASE(vect_complex_mul, vect_complex_s32_mul_random);
}

TEST_GROUP(vect_complex_mul);
TEST_SETUP(vect_complex_mul) { fflush(stdout); }
TEST_TEAR_DOWN(vect_complex_mul) {}
...
TEST(vect_complex_mul, vect_complex_s16_mul_prepare)
{
    ...
    for(int r = 0; r < REPS; r++){
        ...
        vect_complex_s16_mul_prepare(&a_exp, &a_shr, b_exp, c_exp, b_hr, c_hr);
...
TEST(vect_complex_mul, vect_complex_s32_mul_prepare)
{
    unsigned seed = SEED_FROM_FUNC_NAME();
    ...
    for(int r = 0; r < REPS; r++){
        ...
        vect_complex_s32_mul(&A, &B, &C, 1, b_shr, c_shr);

        TEST_ASSERT_EQUAL( INT32_MAX, A.im );
        vect_complex_s32_mul(&A, &B, &C, 1, b_shr+1, c_shr);
...
TEST(vect_complex_mul, vect_complex_s16_mul_random)
{
    unsigned seed = SEED_FROM_FUNC_NAME();
...
TEST(vect_complex_mul, vect_complex_s32_mul_basic)
{
    ...
    for(unsigned int v = start_case; v < N_cases; v++){
        ...
        for( int l = 0; l < sizeof(lengths)/sizeof(lengths[0]); l++){
            ...
            hr = vect_complex_s32_mul(A, B, C, len, casse->shr.b, casse->shr.c);

            for(unsigned int i = 0; i < len; i++){
                TEST_ASSERT_EQUAL_MSG(casse->expected.re, A[0].re, casse->line);
                TEST_ASSERT_EQUAL_MSG(casse->expected.im, A[0].im, casse->line);
            }
            TEST_ASSERT_EQUAL_MSG(vect_s32_headroom((int32_t*) A, 2*len), hr, casse->line);

            memcpy(A, B, sizeof(A));
            hr = vect_complex_s32_mul(A, A, C, len, casse->shr.b, casse->shr.c);
            ...
            memcpy(A, C, sizeof(A));
            hr = vect_complex_s32_mul(A, B, A, len, casse->shr.b, casse->shr.c);
...
TEST(vect_complex_mul, vect_complex_s32_mul_random)
{
    ...
    for(unsigned int v = 0; v < REPS; v++){
        ...
        hr = vect_complex_s32_mul(A, B, C, len, b_shr, c_shr);
        ...
        memcpy(&A, &B, sizeof(A));
        hr = vect_complex_s32_mul(A, A, C, len, b_shr, c_shr);
        ...
        memcpy(&A, &C, sizeof(A));
        hr = vect_complex_s32_mul(A, B, A, len, b_shr, c_shr);
