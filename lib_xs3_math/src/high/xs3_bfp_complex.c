...
headroom_t bfp_cls_vect_complex_s16(
    bfp_complex_s16_t* a)
{
    const headroom_t re_hr = xs3_cls_array_s16(a->real, a->length);
    const headroom_t im_hr = xs3_cls_array_s16(a->imag, a->length);
    ...
    const exponent_t exp)
{
    ...
    const headroom_t re_hr = xs3_shl_vect_s16(a->real, b->real, b->length, exp);
    const headroom_t im_hr = xs3_shl_vect_s16(a->imag, b->imag, b->length, exp);
    a->hr = (re_hr <= im_hr)? re_hr : im_hr;
}
    ...
    const bfp_complex_s32_t* c)
{
    ...
    a->length = b->length;
    a->exp = a_exp;
    a->hr = xs3_add_vect_s32((int32_t*) a->data, (int32_t*) b->data, (int32_t*) c->data, 2 * b->length, b_shr, c_shr);
}
    ...
    const bfp_complex_s32_t* c)
{
    ...
    a->length = b->length;
    a->exp = a_exp;
    a->hr = xs3_sub_vect_s32((int32_t*) a->data, (int32_t*) b->data, (int32_t*) c->data, 2 * b->length, b_shr, c_shr);
}
