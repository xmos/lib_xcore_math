    ...
    const int32_t x[32]) 
{
  ...
  ////// Split/move data
  xs3_vect_s32_flip(&buff[0], &x[HALF_DCT_N], HALF_DCT_N);

  ////// Sum and differences
  xs3_dct_adsb_s32(&y[0], &buff[0], &x[0], &buff[0], HALF_DCT_N_CHUNKS, dct_lut);
  ...
  ////// Down-shift
  xs3_vect_s32_shr(&y[HALF_DCT_N], &y[HALF_DCT_N], HALF_DCT_N, 2);
  xs3_vect_s32_shr(&buff[0], &buff[0], HALF_DCT_N, 1);


  ////// Interleave
  xs3_dct_deconvolve_s32(&y[0], &y[HALF_DCT_N], &buff[0], DCT_N);
}
    ...
    const int32_t x[48])
{
  ...
  xs3_vect_s32_flip(&buff[0], &x[24], 24);

  ////// Sum and differences
  // the sums[] will go into y[0:24] and differences in buff[0:24]
  xs3_dct_adsb_s32(&y[0], &buff[0], &x[0], &buff[0], 3, dct48_lut);
  ...
  xs3_vect_s32_shr(&y[24], &y[24], 24, 2);
  xs3_vect_s32_shr(&buff[0], &buff[0], 24, 1);
  ...
  xs3_dct_deconvolve_s32(&y[0], &y[24], &buff[0], 48);
}
