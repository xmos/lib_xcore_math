    ...
    const unsigned mode_bits)
{
  ...
  switch(mode_bits){
    case 0x0000: // 32-bit
      return 31 - xs3_vect_s32_headroom(a, length_bytes >> 2);
      break;
    case 0x0100: // 16-bit
      return 15 - xs3_vect_s16_headroom((void*) a, length_bytes >> 1);
      break;
    case 0x0200: // 8-bit
      // @todo: There is currently no xs3_vect_s8_headroom()
      assert(0); // Not supported on x86 at the moment.
      // return 7 - xs3_vect_s8_headroom((void*) a, length_bytes);
      break;
    default:
      assert(0); // shouldn't happen
  }
