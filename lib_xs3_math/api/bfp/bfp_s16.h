...
/*
    ...
    Where specified, the VPU-enhanced arithmetic functions apply symmetric saturation logic, which
    is applied where results would otherwise overflow.

    The resulting integer range for N-bit saturation is   -(2^(N-1))+1  to  (2^(N-1))-1.

    8-bit:      (-127, 127) 16-bit:     (-65535, 65535) 32-bit:     (-2147483647, 2147483647)
*/

/**
 * @page page_bfp_s16_h  bfp_s16.h
 * 
 * This header contains functions implementing arithmetic operations on 16-bit block floating-point
 * vectors.
 * 
