...
def main():
    ...
    parser.add_argument("--dit", action="store_true", default=False,
    ...
    with open(source_filename, "w+") as source_file:
        with open(header_filename, "w+") as header_file:
            ...
            if args.dit:
                ...
                table_name = generate_DIT_FFT(args.max_fft_log2, header_file, source_file, args, M=32)
            ...
            if args.dif:
                ...
                table_name = generate_DIF_FFT(args.max_fft_log2, header_file, source_file, args, M=32)
...
def generate_DIT_FFT(N, header_file, source_file, args, M=8*4):
    ...
    header_file.write(f"extern const complex_s32_t {table_name}[{4*twiddle_factor_count}]; // {total_bytes} bytes\n")
    ...
    twiddle_factors_int = np.asarray(np.rint(r*2**-twiddle_exponent), dtype=np.int32)

    twiddle_factors_int = np.reshape(twiddle_factors_int, (twiddle_factor_count * 4, 2))

    source_file.write(f"const complex_s32_t {table_name}[{4*twiddle_factor_count}] = \n\t{{\n\t")

    for t in range(len(twiddle_factors_int)):
        source_file.write("{%11d, %11d}, "%(twiddle_factors_int[t][0], twiddle_factors_int[t][1]))
        if (t+1) % 4 == 0:
            source_file.write("\n\t")
