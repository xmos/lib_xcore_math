...
#define MAX_PROC_FRAME_LENGTH_LOG2 (XS3_MAX_DIF_FFT_LOG2)
...
void test_xs3_fft_dif_forward_complete()
{
    ...
    for(unsigned k = MIN_FFT_N_LOG2; k <= MAX_PROC_FRAME_LENGTH_LOG2; k++){
        ...
        for(unsigned t = 0; t < (1 << LOOPS_LOG2); t++){
            ...
            xs3_fft_dif_forward(a, FFT_N, &headroom, XS3_DIF_FFT_LUT(FFT_N), &exponent);
...
void test_xs3_fft_dif_inverse_complete()
{
    ...
    for(unsigned k = MIN_FFT_N_LOG2; k <= MAX_PROC_FRAME_LENGTH_LOG2; k++){
        ...
        for(unsigned t = 0; t < (1<<LOOPS_LOG2); t++){
            ...
            xs3_fft_dif_inverse(a, FFT_N, &headroom, XS3_DIF_FFT_LUT(FFT_N), &exponent);
            xs3_fft_index_bit_reversal(a, FFT_N);
