...
void test_bfp_fft_forward_complex()
{
    ...
    for(unsigned k = MIN_FFT_N_LOG2; k <= MAX_PROC_FRAME_LENGTH_LOG2; k++){
        ...
        for(unsigned t = 0; t < (1<<LOOPS_LOG2); t++){ 
            ...
            bfp_init_vect_complex_s32(&A, a, initial_exponent, FFT_N, 1);
...
void test_bfp_fft_forward_stereo()
{
    ...
    for(unsigned k = MIN_FFT_N_LOG2; k <= MAX_PROC_FRAME_LENGTH_LOG2; k++){
        ...
        for(unsigned t = 0; t < (1<<LOOPS_LOG2); t++){
            ...
            bfp_init_vect_ch_pair_s32(&A, a, initial_exponent, FFT_N, 1);
...
void test_bfp_fft_inverse_stereo()
{
    ...
    for(unsigned k = MIN_FFT_N_LOG2; k <= MAX_PROC_FRAME_LENGTH_LOG2; k++){
        ...
        for(unsigned t = 0; t < (1<<LOOPS_LOG2); t++){
            ...
            bfp_init_vect_complex_s32(&A_fft, &ab_data[0], initial_exponent, FFT_N/2, 1);
            bfp_init_vect_complex_s32(&B_fft, &ab_data[FFT_N/2], initial_exponent, FFT_N/2, 1);

            bfp_init_vect_ch_pair_s32(&AB, (ch_pair_s32_t*) ab_data, 0, FFT_N, 0);
...
void test_bfp_fft_forward_mono()
{
    ...
    for(unsigned k = MAX(MIN_FFT_N_LOG2, 4); k <= MAX_PROC_FRAME_LENGTH_LOG2; k++){
        ...
        for(unsigned t = 0; t < (1<<LOOPS_LOG2); t++){
            ...
            bfp_init_vect_s32(&A, a, initial_exponent, FFT_N, 1);
