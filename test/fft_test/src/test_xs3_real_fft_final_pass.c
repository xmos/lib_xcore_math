...
void test_xs3_real_fft_final_pass_s32()
{
    ...
    for(unsigned k = 4; k <= MAX_PROC_FRAME_LENGTH_LOG2; k++){
        ...
        for(unsigned t = 0; t < (1 << LOOPS_LOG2); t++){
            ...
            xs3_bit_reverse_indexes((complex_s32_t*) a, N/2);
            xs3_fft_dit_s32((complex_s32_t*) a, N/2, &headroom, xs3_dit_fft_lut, &exponent);
            ...
            xs3_real_fft_final_pass_s32((complex_s32_t*) a, N, XS3_DIT_REAL_FFT_LUT(N));
...
#if TIME_FUNCS
        printf("\txs3_real_fft_final_pass_s32 (%u-point): %f us\n", N, worst_timing);
#endif
...
void test_xs3_real_fft_final_pass()
{
    ...
    RUN_TEST(test_xs3_real_fft_final_pass_s32);
}