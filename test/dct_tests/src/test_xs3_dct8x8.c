...
TEST_GROUP_RUNNER(xs3_dct8x8) {
  RUN_TEST_CASE(xs3_dct8x8, dct8x8_stageA);
  RUN_TEST_CASE(xs3_dct8x8, dct8x8_stageB);
  RUN_TEST_CASE(xs3_dct8x8, dct8x8_forward);
  RUN_TEST_CASE(xs3_dct8x8, dct8x8_inverse);
}

TEST_GROUP(xs3_dct8x8);
TEST_SETUP(xs3_dct8x8) { fflush(stdout); }
TEST_TEAR_DOWN(xs3_dct8x8) {}
...
#define FUNC_NAME "dct8x8_stageA"
  ...
  for(unsigned t = 0; t < (1<<LOOPS_LOG2); t++){
    ...
    headroom_t actual_hr = xs3_vect_s16_headroom(&y[0][0], 64);
  ...
  for(unsigned t = 0; t < (1<<LOOPS_LOG2); t++){
    ...
    for(int row = 0; row < 8; row++){
      for(unsigned col = 0; col < 8; col++){
        ...
        // NOTE: currently no xs3_vect_s8_headroom() function.
...
TEST(xs3_dct8x8, dct8x8_forward)
{
...
TEST(xs3_dct8x8, dct8x8_inverse)
{
