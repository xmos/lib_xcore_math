...
TEST_GROUP_RUNNER(xs3_vect_complex_add_scalar) {
  RUN_TEST_CASE(xs3_vect_complex_add_scalar, xs3_vect_complex_s16_add_scalar);
  RUN_TEST_CASE(xs3_vect_complex_add_scalar, xs3_vect_complex_s32_add_scalar);
}

TEST_GROUP(xs3_vect_complex_add_scalar);
TEST_SETUP(xs3_vect_complex_add_scalar) { fflush(stdout); }
TEST_TEAR_DOWN(xs3_vect_complex_add_scalar) {}
...
TEST(xs3_vect_complex_add_scalar, xs3_vect_complex_s16_add_scalar)
{
  ...
  for(int v = 0; v < REPS; v++){
    ...
    hr = xs3_vect_complex_s16_add_scalar(A_real, A_imag, B_real, B_imag, C, len, b_shr);

    TEST_ASSERT_EQUAL_MESSAGE( xs3_vect_complex_s16_headroom(A_real, A_imag, len), hr, "Headroom is wrong.");
...
TEST(xs3_vect_complex_add_scalar, xs3_vect_complex_s32_add_scalar)
{
  ...
  for(int v = 0; v < REPS; v++){
    ...
    hr = xs3_vect_complex_s32_add_scalar(A, B, C, len, b_shr);

    TEST_ASSERT_EQUAL_MESSAGE( xs3_vect_complex_s32_headroom(A, len), hr, "Headroom is wrong.");
