...
TEST_GROUP_RUNNER(xs3_vect_f32_dot) {
  RUN_TEST_CASE(xs3_vect_f32_dot, xs3_vect_f32_dot);
}

TEST_GROUP(xs3_vect_f32_dot);
TEST_SETUP(xs3_vect_f32_dot) { fflush(stdout); }
TEST_TEAR_DOWN(xs3_vect_f32_dot) {}
...
TEST(xs3_vect_f32_dot, xs3_vect_f32_dot)
{
  ...
  // xs3_vect_f32_dot() traces different code paths depending upon the
  ...
  for(unsigned B_OFF = 0; B_OFF <= 1; B_OFF++){
    for(unsigned C_OFF = 0; C_OFF <= 1; C_OFF++){
      ...
      for(int v = 0; v < REPS/4; v++){
        ...
        float result = xs3_vect_f32_dot(&b[B_OFF], &c[C_OFF], len);
