...
TEST_GROUP_RUNNER(xs3_chunk_s16_accumulate) {
  RUN_TEST_CASE(xs3_chunk_s16_accumulate, xs3_chunk_s16_accumulate_NONPOS_SHR);
  RUN_TEST_CASE(xs3_chunk_s16_accumulate, xs3_chunk_s16_accumulate_POS_SHR);
}

TEST_GROUP(xs3_chunk_s16_accumulate);
TEST_SETUP(xs3_chunk_s16_accumulate) { fflush(stdout); }
TEST_TEAR_DOWN(xs3_chunk_s16_accumulate) {}
...
TEST(xs3_chunk_s16_accumulate, xs3_chunk_s16_accumulate_NONPOS_SHR)
{
  unsigned seed = SEED_FROM_FUNC_NAME();

  int16_t DWORD_ALIGNED B[VPU_INT16_EPV];
  xs3_split_acc_s32_t DWORD_ALIGNED acc;
  ...
  for(int v = 0; v < REPS; v++){
    ...
    xs3_vect_s32_split_accs(&acc, expected, length);
    ...
    vpu_ctrl = xs3_chunk_s16_accumulate(&acc, B, b_shr, vpu_ctrl);

    xs3_vect_s32_merge_accs(actual, &acc, length);
...
TEST(xs3_chunk_s16_accumulate, xs3_chunk_s16_accumulate_POS_SHR)
{
  unsigned seed = SEED_FROM_FUNC_NAME();
