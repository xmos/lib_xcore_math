...
def array_to_string(arr, val_fmtr=str):
    if len(arr.shape) == 1:
        ...
        return "{" + ",".join([array_to_string(arr[i], val_fmtr) for i in range(arr.shape[0])]) + "}"
...
print("    ",end=""); print_arr_c("samples", samples.astype(np.int64))
print("    ",end=""); print_arr_c("coef", pcoef, val_fmtr=hex)
print("    ",end=""); print_arr_c("Y_exp", y_q30.astype(np.int64))


# print( "    xs3_biquad_filter_s32_t filter;")
