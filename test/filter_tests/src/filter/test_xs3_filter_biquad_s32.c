...
TEST_GROUP_RUNNER(xs3_filter_biquad_s32) {
  RUN_TEST_CASE(xs3_filter_biquad_s32, case0);
  RUN_TEST_CASE(xs3_filter_biquad_s32, case1);
  RUN_TEST_CASE(xs3_filter_biquad_s32, case2);
  RUN_TEST_CASE(xs3_filter_biquad_s32, case3);
}

TEST_GROUP(xs3_filter_biquad_s32);
TEST_SETUP(xs3_filter_biquad_s32) { fflush(stdout); }
TEST_TEAR_DOWN(xs3_filter_biquad_s32) {}

static char msg_buff[200];

#define MUL32(X, Y)     (((((int64_t)(X)) * (Y)) + (1<<29)) >> 30)

TEST(xs3_filter_biquad_s32, case0)
{
    ...
    int32_t res = xs3_filter_biquad_s32(&filter, 100);
    TEST_ASSERT_EQUAL(100, res);

    res = xs3_filter_biquad_s32(&filter, 50);
    TEST_ASSERT_EQUAL(250, res);

    res = xs3_filter_biquad_s32(&filter, 4000);
    TEST_ASSERT_EQUAL(4500, res);

}

TEST(xs3_filter_biquad_s32, case1)
{
    xs3_biquad_filter_s32_t filter;
...
TEST(xs3_filter_biquad_s32, case2)
{
    xs3_biquad_filter_s32_t filter;
...
TEST(xs3_filter_biquad_s32, case3)
{
    ...
    for(int i = 0; i < N_samples; i++){
        int32_t y = xs3_filter_biquad_s32(&filter, samples[i]);
