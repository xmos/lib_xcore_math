 ...
 * @returns     Next filtered output sample
 * 
 * @note When the result exceedes the 32-bit range, the output will overflow.
 * 
 ...
 * at the cost of several compute cycles. The cost will depend on the number of biquads and the target artitecture.
 * 
 * @param[inout]    filter          Filter to be processed
 * @param[in]       new_sample      New input sample to be processed by `filter`
 * 
