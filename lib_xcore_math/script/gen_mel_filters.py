...
# python gen_mel_filters.py -fft_size 512 -mel_size=24 -path=. -type=compressed

def smaller_filterbank(filterbank):
    starts = (filterbank != 0).argmax(axis=1)
    lens = (filterbank[:, starts] == 0).argmax(axis=1)
    n_filts = filterbank.shape[0]
    compact_filts = []
    alt_lens = []
    for i in range(n_filts):
        this_len = (filterbank[i, starts[i]:] == 0).argmax()
        lens[i] = this_len
        alt_lens.append(len(numpy.nonzero(filterbank[i])[0]))

        compact_filts.append(filterbank[i][starts[i]:starts[i] + this_len])

    return compact_filts, starts, lens
...
def get_filterbanks(nfilt=20,nfft=512,samplerate=16000,lowfreq=0,highfreq=None, xmos_impl=False):
    ...
    if xmos_impl:
        ...
        fbank = lr.filters.mel(sr=samplerate, n_fft=nfft, n_mels=nfilt, fmin=lowfreq, fmax=highfreq, htk=False, norm="slaney")
        _, bin, _ = smaller_filterbank(fbank)
...
def apply_full_mel(bins, fbank):
    ...
    return filtered
FUDGE_FACTOR = 1 # sorry software engineers
...
def gen_c_line(float_data, mel_max):
    ...
    for data in float_data:
        ...
        line += f"{int32_data * FUDGE_FACTOR}, "
    line = line[:-1] + "\n"
    return line
...
class compact_mel:
    def __init__(self, fbank, filter_start_bins):
        ...
        self.num_mels = num_mels - 2 #because we add an extra two during generation
    ...
    def gen_c_src(self, var_name):
        ...
        c_text += "\n"
        c_text += f"q1_31 {array_name}[{len(self.flattened_fbank)}] = {{\n"
...
class compressed_mel:
    def __init__(self, fbank):
        ...
        for idx in range(0, num_mels):
            ...
            compressed_mel.append((shortended_line[1:], first_non_zero_idx, next_zero_idx_or_end - first_non_zero_idx))
...
def main():
    ...
    parser.add_argument('-fft_size', action="store", type=int, default=512)
    parser.add_argument('-mel_size', action="store", type=int, default=64)
    parser.add_argument('-path', action="store", default='.')
    parser.add_argument('-type', action="store", default='compressed')
    parser.add_argument('-xmos', action='store_true', default=False)

    args = parser.parse_args()

    fbank, band_start_bins = get_filterbanks(args.mel_size, args.fft_size, 16000, xmos_impl=args.xmos)
    # fbank_standard = fbank[ :-1]
